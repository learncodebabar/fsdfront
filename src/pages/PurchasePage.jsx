            ...
            >
              ...
              {rows.map((r, i) => (
                ...
                >
                  ...
                  <td className="r" style={{ color: "#888", fontSize: 11 }}>
                    {fmt(r._saleRate)}
                  </td>
...
function printBill(bill) {
  ...
  <style>body{font-family:Arial,sans-serif;font-size:12px;padding:20px}h2{text-align:center}h3{text-align:center;color:#555;font-size:11px;margin:2px 0 10px;letter-spacing:1px}
  ...
  table{width:100%;border-collapse:collapse}th{background:#8e44ad;color:#fff;padding:5px 7px;border:1px solid #6c3483}
  td{border:1px solid #ccc;padding:4px 7px}tr:nth-child(even)td{background:#f9f5ff}
  ...
  </head><body><h2>${SHOP}</h2><h3>◆ PURCHASE BILL ◆</h3>
  <div class="meta">
    <span><b>Bill #:</b> ${bill.invoiceNo}</span><span><b>Date:</b> ${bill.date}</span>
    ${bill.suppName ? `<span><b>Supplier:</b> ${bill.suppName}${bill.suppPhone ? " | " + bill.suppPhone : ""}</span>` : ""}
    ${bill.suppInvoice ? `<span><b>Supp. Invoice:</b> ${bill.suppInvoice}</span>` : ""}
    <span><b>Payment:</b> ${bill.payMode}</span>
  </div>
  ...
  <div class="tots">
    <div class="tr"><span>Sub Total</span><span>Rs. ${Number(bill.subTotal).toLocaleString()}</span></div>
    ${(bill.discAmt || 0) > 0 ? `<div class="tr" style="color:red"><span>Discount</span><span>-Rs. ${Number(bill.discAmt).toLocaleString()}</span></div>` : ""}
    <div class="tr b"><span>Net Total</span><span>Rs. ${Number(bill.netTotal).toLocaleString()}</span></div>
    ${(bill.cashPaid || 0) > 0 ? `<div class="tr" style="color:green"><span>Paid</span><span>Rs. ${Number(bill.cashPaid).toLocaleString()}</span></div>` : ""}
    ${(bill.balance || 0) > 0 ? `<div class="tr b" style="color:red"><span>Balance</span><span>Rs. ${Number(bill.balance).toLocaleString()}</span></div>` : ""}
  </div>
  ${bill.remarks ? `<p style="clear:both;margin-top:8px;font-size:11px;color:#555"><b>Remarks:</b> ${bill.remarks}</p>` : ""}
...
function shareWA(bill) {
  const lines = (bill.items || [])
    ...
    .join("\n");
  const msg =
    `*${SHOP}*\n📦 *Purchase Bill — ${bill.invoiceNo}*\n📅 ${bill.date}\n` +
    (bill.suppName
      ? `🏪 ${bill.suppName}${bill.suppPhone ? "  📞 " + bill.suppPhone : ""}\n`
      : "") +
    (bill.suppInvoice ? `📋 Supp.Inv: ${bill.suppInvoice}\n` : "") +
    `${"─".repeat(26)}\n${lines}\n${"─".repeat(26)}\n` +
    `*Net Total: Rs. ${Number(bill.netTotal).toLocaleString()}*` +
    ((bill.balance || 0) > 0
      ? `\n⚠️ Balance: Rs. ${Number(bill.balance).toLocaleString()}`
      : "");
...
export default function PurchasePage() {
  ...
  // ── Supplier search — same logic as DebitSalePage ──────────────────────
  const handlePhoneSearch = async () => {
    if (!suppPhone.trim() || suppPhone.trim().length < 3) return;
    setPhoneLoad(true);
    try {
      const { data } = await api.get(
        `${EP.CUSTOMERS.GET_ALL}?search=${encodeURIComponent(suppPhone.trim())}&type=supplier`,
      );
      if (data.success && data.data.length > 0) {
        const list = data.data;
        // Exact phone match → auto select
        const exact = list.find((c) => c.phone?.trim() === suppPhone.trim());
        if (exact) {
        ...
        // Multiple → select first (can add modal later)
        selectSupplier(list[0]);
        setPhoneLoad(false);
        return;
      }
      // No match → auto create supplier
  ...
  const selectSupplier = (c) => {
    setSupplier(c);
    setSuppPhone(c.phone || "");
    showMsg(
      `✅ ${c.name}${(c.currentBalance || 0) > 0 ? " — Balance: Rs." + fmt(c.currentBalance) : ""}`,
    );
  };
  ...
  const handleProductSelect = (product) => {
    ...
    const rate = product._rate || 0; // purchaseRate
    setRows((prev) => {
      const next = [...prev];
      next[activeRow] = {
        ...next[activeRow],
  ...
  const onCodeBlur = async (i, code) => {
    ...
    if (found) {
      ...
      const desc = [found.category, found.description, found.company]
        .filter(Boolean)
        .join(" ");
      const rate = pk.purchaseRate || pk.saleRate || 0;
      setRows((prev) => {
        const next = [...prev];
        next[i] = {
          ...next[i],
  ...
  const resumeHold = (id) => {
    ...
    setSuppPhone(h.suppPhone);
    setSuppInvoice(h.suppInvoice);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        customerId: supplier?._id || undefined,
        customerName: supplier?.name || "COUNTER PURCHASE",
        customerPhone: supplier?.phone || suppPhone || "",
        items: validRows,
        subTotal,
      ...
      if (data.success) {
        const bill = {
          ...
          suppPhone: supplier?.phone || suppPhone,
          suppInvoice,
          payMode: actualMode,
          items: validRows,
          subTotal,
  ...
  return (
    <div className="rp-page">
      ...
      {showInvoice && savedBill && (
        ...
        >
          <div className="xp-modal xp-modal-md">
            <div className="xp-modal-tb">
              <span className="xp-modal-title">
                📦 {savedBill.invoiceNo} — Purchase Saved
              </span>
              <button
            ...
            <div style={{ padding: "20px", textAlign: "center" }}>
              ...
              <div style={{ marginBottom: 4 }}>
                <b>{savedBill.invoiceNo}</b> | {savedBill.date}
              </div>
              {savedBill.suppName && (
              ...
              <div style={{ marginBottom: 16, fontSize: 15 }}>
                Net Total: <b>Rs. {fmt(savedBill.netTotal)}</b>
              </div>
              <div
              ...
              >
                <button
                  ...
                  onClick={() => printBill(savedBill)}
                >
                  🖨 Print
                </button>
                <button
                  className="xp-btn xp-btn-wa xp-btn-sm"
                  onClick={() => shareWA(savedBill)}
                >
                  📱 WhatsApp
                </button>
                <button
      ...
      <div className="rp-titlebar">
        ...
        <span className="rp-title-text">Purchase — {SHOP}</span>
      ...
      <div className="rp-body">
        <div className="rp-left">
          ...
          <div className="rp-supp-bar">
            ...
            <span className="rp-label" style={{ marginLeft: 6 }}>
              📋 Supp. Invoice
            </span>
            <input
              ref={invRef}
              className="rp-inv-input"
              value={suppInvoice}
            ...
            {supplier ? (
              <div className="rp-supp-tag">
                ...
                {(supplier.currentBalance || 0) > 0 && (
                  <span className="rp-due">
                    Bal: <b>Rs.{fmt(supplier.currentBalance)}</b>
                  </span>
                )}
                <button
              ...
              <span className="rp-no-supp">
                No supplier selected (optional)
              </span>
            )}
            {holds.map((h) => (
              <button
          ...
          <div className="rp-footer">
            <div className="rp-footer-left">
              <div className="rp-pay-section">
                <div className="rp-pay-title">💳 Payment Mode</div>
            ...
            <div className="rp-totals">
              ...
              <div className="rp-tot-row bold">
                <span>Net Total</span>
                <span className="xp-amt" style={{ color: "#6c3483" }}>
                  Rs. {fmt(netTotal)}
                </span>
              </div>
              {cashAmt > 0 && (
              ...
              {balance > 0 && (
                <div className="rp-tot-row bold">
                  <span>Balance</span>
                  <span className="xp-amt danger">{fmt(balance)}</span>
                </div>
              )}
              <div
                className="rp-tot-row"
        ...
        <div className="rp-right">
          <div className="rp-right-title">📦 Purchase</div>
          <div className="rp-info-cards">
            <div className="rp-info-card">
              <div className="rp-ic-l">Bill #</div>
            ...
            <div className="rp-info-card">
              ...
              <div className="rp-ic-v" style={{ color: "#6c3483" }}>
                Rs.{fmt(netTotal)}
              </div>
            </div>
            {balance > 0 && (
              <div className="rp-info-card danger">
          ...
          <div className="rp-right-desc">
            <p>
              <b>Purchase</b> — Products khareedna supplier se.
            </p>
            <ul>
              <li>Enter on last disc → new row</li>
              <li>F3 → product search (purchase rate)</li>
              <li>F8 → hold bill</li>
              <li>F5 → save & print</li>
            </ul>
          </div>
          {supplier && (
            <div className="rp-supp-summary">
              ...
              {suppInvoice && (
                <div className="rp-ss-phone">📋 Inv: {suppInvoice}</div>
              )}
              {(supplier.currentBalance || 0) > 0 && (
      ...
      <div className="xp-statusbar">
        <div className="xp-status-pane">📦 Purchase | {invoiceNo || "—"}</div>
