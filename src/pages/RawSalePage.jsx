// RawSalePage.jsx — Raw / Wholesale Sale
...
function printBill(sale) {
  ...
  </head><body><h2>${SHOP}</h2><h3>◆ RAW SALE INVOICE ◆</h3>
  <div class="meta"><span><b>Invoice:</b> ${sale.invoiceNo}</span><span><b>Date:</b> ${sale.invoiceDate}</span>
  ${sale.customerName && sale.customerName !== "COUNTER SALE" ? `<span><b>Customer:</b> ${sale.customerName}${sale.customerPhone ? " | " + sale.customerPhone : ""}</span>` : ""}
...
export default function RawSalePage() {
  ...
  }, [rows, customer, cashPaid, extraDisc, remarks]);
  ...
  const handlePhoneSearch = async () => {
    ...
    try {
      const { data } = await api.get(
        `${EP.CUSTOMERS.GET_ALL}?search=${encodeURIComponent(phone.trim())}&type=raw-sale`,
      );
      if (data.success && data.data.length > 0) {
        ...
        // Multiple → just select first (no modal needed for raw sale)
        if (list.length === 1) {
          selectCustomer(list[0]);
          return;
        }
        // Multiple matches — select first exact partial
        selectCustomer(list[0]);
      } else {
        await createWalkinCustomer();
      }
  ...
  const selectCustomer = (c) => {
    ...
    setPhone(c.phone || "");
    showMsg(
      `✅ ${c.name}${(c.currentBalance || 0) > 0 ? " — Due: Rs." + fmt(c.currentBalance) : ""}`,
    );
  };
  ...
  return (
    <div className="rs-page">
      ...
      <div className="rs-titlebar">
        ...
        <span className="rs-title-text">Raw Sale — {SHOP}</span>
        <div className="rs-title-fields">
          <div className="rs-hf">
            <span>Invoice #</span>
            <input
              className="rs-hinput bold"
              value={invoiceNo}
              readOnly
              tabIndex={-1}
            />
      ...
      <div className="rs-body">
        ...
        <div className="rs-left">
          {/* Customer bar */}
          <div className="rs-cust-bar">
            <span className="rs-label">👤 Customer</span>
            <input
              ref={phoneRef}
            ...
            {customer ? (
              <div className="rs-cust-tag">
                <span>
                  ✅ <b>{customer.name}</b>
                </span>
                {(customer.currentBalance || 0) > 0 && (
                  <span className="rs-due">
                    Due: <b>Rs.{fmt(customer.currentBalance)}</b>
                  </span>
                )}
                <button
              ...
              <span className="rs-no-cust">Walk-in / Counter Sale</span>
            )}
            {holds.map((h) => (
              <button
                key={h.id}
                className="rs-hold-pill"
          ...
          <div className="rs-search-bar">
            <span className="rs-label">📦 Product</span>
            <input
              className={`rs-search-input${searchText ? " filled" : ""}`}
              value={searchText}
          ...
          <div className="rs-footer">
            <div className="rs-footer-left">
              {/* Payment */}
              <div className="rs-pay-section">
                <div className="rs-pay-title">💳 Payment</div>
                <div className="rs-pay-modes">
                  {[
                    ["Cash", "rs-pay-cash"],
                    ["Credit", "rs-pay-credit"],
                    ["Bank", "rs-pay-bank"],
                    ["Cheque", "rs-pay-cheque"],
                  ].map(([mode, cls]) => (
                    <button
                      key={mode}
                      className={`rs-pay-btn ${payMode === mode ? cls + " active" : ""}`}
                      onClick={() => {
                        setPayMode(mode);
                ...
                <div className="rs-pay-row">
                  <label>Cash Received</label>
                  <input
                    ref={cashRef}
                    type="number"
                    className="rs-pay-input"
                    value={cashPaid}
                    onChange={(e) => {
            ...
            <div className="rs-totals">
              ...
              <div className={`rs-tot-row bold ${balance > 0 ? "danger" : ""}`}>
                <span>Balance</span>
        ...
        <div className="rs-right">
          <div className="rs-right-title">🏷 Raw Sale</div>
          <div className="rs-info-cards">
            <div className="rs-info-card">
              <div className="rs-ic-l">Invoice</div>
              <div className="rs-ic-v">{invoiceNo || "—"}</div>
            </div>
            <div className="rs-info-card">
              <div className="rs-ic-l">Items</div>
              <div className="rs-ic-v">
                {rows.filter((r) => r.description).length}
              </div>
            </div>
            <div className="rs-info-card">
              <div className="rs-ic-l">Net Total</div>
              <div className="rs-ic-v" style={{ color: "var(--xp-blue-dark)" }}>
                Rs.{fmt(netTotal)}
              </div>
            </div>
            <div className={`rs-info-card ${balance > 0 ? "danger" : ""}`}>
              <div className="rs-ic-l">Balance</div>
              <div className="rs-ic-v">{fmt(balance)}</div>
            </div>
          </div>
          <div className="rs-right-desc">
            <p>
              <b>Raw Sale</b> — Direct wholesale / bulk sale to buyers.
            </p>
            <p>Stock is deducted. Records saved for history.</p>
            <ul>
          ...
          {customer && (
            <div className="rs-cust-summary">
              <div className="rs-cs-name">{customer.name}</div>
              {customer.phone && (
                <div className="rs-cs-phone">📞 {customer.phone}</div>
              )}
              {(customer.currentBalance || 0) > 0 && (
                <div className="rs-cs-balance">
                  Current Due:{" "}
                  <span style={{ color: "var(--xp-red)", fontWeight: "bold" }}>
                    Rs. {fmt(customer.currentBalance)}
                  </span>
                </div>
              )}
            </div>
      ...
      <div className="xp-statusbar">
        <div className="xp-status-pane">🏷 Raw Sale | {invoiceNo || "—"}</div>
        <div className="xp-status-pane">👤 {customer?.name || "Walk-in"}</div>
